...
import {
  ...
  VJGestureAction,
  GestureType,
} from '@/utils/aiGestureRecognition';
      ...
      {gesture ? (
        <div className="space-y-4">
          <div className="text-center">
            <div className="text-6xl mb-2">{getGestureEmoji(gesture.gesture)}</div>
            <div className="text-2xl font-semibold">{gesture.gesture}</div>
...
export const AIGestureDemo: React.FC = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div className="text-center">
        <h2 className="text-3xl font-bold text-gray-800 mb-2">🤖 AI Gesture Recognition Demo</h2>
        <p className="text-gray-600">VJ操作のためのリアルタイムジェスチャー認識システム</p>
      </div>
            ...
            {actionHistory.length > 0 ? (
              actionHistory.map((action, index) => (
                <div key={index} className="p-2 bg-blue-50 rounded">
                  <div className="font-medium text-blue-800">{action.type.replace('_', ' ')}</div>
      ...
      <div className="text-center text-sm text-gray-500">
        <p>
          💡 ヒント:
          カメラに向かってジェスチャーをしてください。認識精度は照明と背景によって変わります。
        </p>
      </div>
    </div>
  );
